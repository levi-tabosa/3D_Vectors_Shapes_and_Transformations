...
class Demo extends JComponent {
   ...
   private void drawShapes(Graphics g) {
      if (shapes != null) {
         ...
         for (int i = 0; i < _shapes.length; i++) {
            ...
            for (int j = 0; j < n; j++) {
               points[j][0] = (int) ((_W >> 1) + (shapes[i][j].coord[0] * near / (shapes[i][j].coord[1] + far)) * _i);
               points[j][1] = (int) ((_H >> 1) + (shapes[i][j].coord[2] * near / (shapes[i][j].coord[1] + far)) * _i);
            }
   ...
   if (_vectors != null) {
         ...
         for (int i = 0; i < _vectors.length; i++) {
            int px = (int) ((_W >> 1) + (vectors[i].coord[0] * near / (vectors[i].coord[1] + far)) * _i),
               py = (int) ((_H >> 1) + (vectors[i].coord[2] * near / (vectors[i].coord[1] + far)) * _i);
   ...
   private void drawLines(Graphics g) {
      ...
      for (int i = 0; i < 3; i++) {
         int px1 = (int) (center_x + (lines[i<<1].coord[0] * near / (lines[i<<1].coord[1] + far)) * _i),
            py1 = (int) (center_y + (lines[i<<1].coord[2] * near / (lines[i<<1].coord[1] + far)) * _i),
            px2 = (int) (center_x + (lines[(i<<1)+1].coord[0] * near / (lines[(i<<1)+1].coord[1] + far)) * _i),
            py2 = (int) (center_y + (lines[(i<<1)+1].coord[2] * near / (lines[(i<<1)+1].coord[1] + far)) * _i);
         g.setColor(new Color(255 - i * 100, i * 110, 22 << i << 1));
         g.drawLine(px1, py1, px2, py2);
         for (int j = -10; j <= 10; j++) { // jank
            g.drawString(j + "", center_x + (px2 - center_x) / 10 * j, center_y + (py2 - center_y) / 10 * j);
         }
      }
      g.setColor(new Color(90, 90, 90, 120));
         for (int i = 0; i < 21; i++) {
            int px1 = (int) (center_x + (gridLines[i<<2].coord[0] * near / (gridLines[i<<2].coord[1] + far)) * _i),
               py1 = (int) (center_y + (gridLines[i<<2].coord[2] * near / (gridLines[i<<2].coord[1] + far)) * _i),
               px2 = (int) (center_x + (gridLines[(i<<2)+1].coord[0] * near / (gridLines[(i<<2)+1].coord[1] + far)) * _i),
               py2 = (int) (center_y + (gridLines[(i<<2)+1].coord[2] * near / (gridLines[(i<<2)+1].coord[1] + far)) * _i);
            g.drawLine(px1, py1, px2, py2);
            px1 = (int) (center_x + (gridLines[(i<<2)+2].coord[0] * near / (gridLines[(i<<2)+2].coord[1] + far)) * _i);
            py1 = (int) (center_y + (gridLines[(i<<2)+2].coord[2] * near / (gridLines[(i<<2)+2].coord[1] + far)) * _i);
            px2 = (int) (center_x + (gridLines[(i<<2)+3].coord[0] * near / (gridLines[(i<<2)+3].coord[1] + far)) * _i);
            py2 = (int) (center_y + (gridLines[(i<<2)+3].coord[2] * near / (gridLines[(i<<2)+3].coord[1] + far)) * _i);
            g.drawLine(px1, py1, px2, py2);
      }
   }
   ...
   SPHERE(96) {
      ...
      public V3[] getVectors() {
         ...
         for (int i = 0; i < res; i++) {
            ...
            for (int j = 0; j < res; j++) {
               //vectors[i * res + j] = Utils.rotZ.apply(Utils.rotX.apply(aux, j * (2 * Math.PI / res)), i * (2 * Math.PI / res));
